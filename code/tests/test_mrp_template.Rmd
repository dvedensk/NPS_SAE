...
Let i be individual, j be PUMA. We first fit logistic regression using NPS data. 
...
Let s be posterior draw and c be cell build from probability sample, $w_c$ is sum of individual weight in the cell c.  
\[p_{c,s} = \text{expit} \left( X_c^{\top} \beta_s \right)\]
The PUMA-level estimate for area \( j \) is then
\[
\theta_{j,s}
=
\frac{
\sum\limits_{c \in j} w_c \, p_{c,s}
}{
\sum\limits_{c \in j} w_c
}.
\]
