...
The current implementation in `code/models/si2.stan` uses a weakly informative fixed-effect prior
$\boldsymbol{\beta} \sim \text{Normal}(0, 3)$, where 3 is the standard deviation.
The PUMA random intercepts use a non-centered parameterization with
$z_m \sim \text{Normal}(0, 1)$ and $\sigma_m \sim \text{Cauchy}^+(0, 5)$.
...
model {
  // Priors: beta_j ~ Normal(0, 3), where 3 is the standard deviation
  beta ~ normal(0, 3);
  z_puma ~ normal(0, 1);
  sigma_puma ~ cauchy(0, 5);

  // Likelihood
  target += reduce_sum(bern_glm_ll_slice, y, grainsize, X, beta, puma_id, a_puma);
}
...
\[p_{j,s} = \text{expit} \left( X_j^{\top} \beta_s + \alpha_{m[j],s} \right)\]
...
# IMPORTANT: Bootstrap is essential for MRP-R to achieve proper coverage.
# Without bootstrap (bootstrap=FALSE), MRP-R only captures posterior uncertainty
# from the Bayesian model parameters, resulting in ~53% coverage instead of the
