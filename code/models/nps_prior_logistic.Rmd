...
```{r}
# TODO: test this
# TODO: better handle the parallel chains parallelization
# TODO: better handle the random seed
power_prior_cmdstanr <- function(
  y,
  X,
  y_NP,
  X_NP,
  ...
  mod <- cmdstan_model("nps_prior_logistic.stan")
  ...
  beta_draws <- fit$draws("beta", format = "matrix")  # (S x p)
  S <- nrow(beta_draws)

  # Posterior predictive for PS units
  eta <- X %*% t(beta_draws)            # n x S
  P <- plogis(eta)
  ...
  # TODO: figure out how the PUMA object ends up here (am I referring to something from the global env or a )
  ...
  res_df <- data.frame(
    ...
    lower_CI = CI[, 1],
    upper_CI = CI[, 2],
    model = "NPS Prior: Power Prior (cmdstanr)"
  )
