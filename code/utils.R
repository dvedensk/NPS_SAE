...
estimate_ipw <- function(ps, nps, cov_formula, method) {
  stopifnot(method %in% c("ignorable", "beta_reg", "weighted", "reff"))
  ...
  if(method == "beta_reg") {
    ...
    combined <- combined %>%
      mutate(
        PWGTP = ifelse(Z == 1, 1, PWGTP),
        PUMA = factor(PUMA)
      )

    nps_re <- nps %>%
      mutate(PUMA = factor(PUMA, levels = levels(combined$PUMA)))

    rhs <- attr(terms(cov_formula), "term.labels")
    rhs <- rhs[rhs != "PUMA"]
