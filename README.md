...
To get started, run `code/download_data.R`, which downloads and unzips California ACS data for people and households from
...
and places these in the `data` directory. 
Next, run `code/process_population.R` to generate the population file. 
The simulation logic is contained in `main.R` and needs to be run for all three settings, 'easy', 'medium', and 'hard'.
